...
public class QueryCLI {
    ...
    private static void readPoint(List<String> hosts, List<String> metrics, long time, String type, Reader reader) {
        switch (type) {
            case "INT":
                for (Map.Entry<String, List<IntPoint>> entry : reader.getIntPoint(hosts, metrics, time).entrySet()) {
                    for (IntPoint point : entry.getValue()) {
                        System.out.println(String.format(OUTPUTFORMAT,entry.getKey(),point.getMetric(),point.getPrimaryTime()) + point.getValue());
                    }
                }
                break;
            case "LONG":
                for (Map.Entry<String, List<LongPoint>> entry : reader.getLongPoint(hosts, metrics, time).entrySet()) {
                ...
                break;
            case "FLOAT":
                for (Map.Entry<String, List<FloatPoint>> entry : reader.getFloatPoint(hosts, metrics, time).entrySet()) {
                ...
                break;
            case "DOUBLE":
                for (Map.Entry<String, List<DoublePoint>> entry : reader.getDoublePoint(hosts, metrics, time).entrySet()) {
                ...
                break;
            case "BOOLEAN":
                for (Map.Entry<String, List<BooleanPoint>> entry : reader.getBooleanPoint(hosts, metrics, time).entrySet()) {
                ...
                break;
            case "STRING":
                for (Map.Entry<String, List<StringPoint>> entry : reader.getStringPoint(hosts, metrics, time).entrySet()) {
                ...
                break;
            case "GEO":
                for (Map.Entry<String, List<GeoPoint>> entry : reader.getGeoPoint(hosts, metrics, time).entrySet()) {
    ...
    private static void readLatest(List<String> hosts, List<String> metrics, String type, Reader reader) {
        switch (type) {
            case "INT":
                for (Map.Entry<String, List<IntPoint>> entry : reader.getIntLatest(hosts, metrics).entrySet()) {
    ...
    private static void readbyRange(List<String> hosts, List<String> metrics, long startTime, long endTime, String type, Reader reader) {
        switch (type) {
            case "INT":
                for (Map.Entry<String, List<IntPoint>> entry : reader.getIntRange(hosts, metrics, startTime, endTime).entrySet()) {
